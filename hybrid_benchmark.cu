...
void run_safe_test(int n, cublasHandle_t handle) {
    ...
    // 33桁データの生成
    void* h_A_f128 = malloc(n_sq * 16);
    void* h_x_f128 = malloc(n * 16);
    float128_t* A_ptr = (float128_t*)h_A_f128;
    float128_t* x_ptr = (float128_t*)h_x_f128;
    for(size_t i=0; i<n_sq; i++) A_ptr[i] = (float128_t)rand()/RAND_MAX + (float128_t)rand()/RAND_MAX * 1e-18Q;
    for(int i=0; i<n; i++) x_ptr[i] = (float128_t)rand()/RAND_MAX + (float128_t)rand()/RAND_MAX * 1e-18Q;
    ...
    free(h_A_f128); free(h_x_f128);
}

int main() {
    cublasHandle_t h; cublasCreate(&h);
    printf("Ozaki Engine [RTX 2060 / 33-Digit Mode]\n");
